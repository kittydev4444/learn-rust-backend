...
fn golf_event(event: GolfEvent) -> String {
    match event {
        ...
        GolfEvent::Drive(yards) => {
            "Golfer just hit the ball ".to_string() + &yards + &" yards!".to_string()
        }
        GolfEvent::ClubSelect { club, max_yard } => {
            "Golfer has equipped ".to_string()
                + &club
                + &". Max yardage is ".to_string()
                + &max_yard
        }
    }
}
...
fn match_golf_example() {
    ...
    println!(
        "{}",
        golf_event(GolfEvent::ClubSelect {
            club: "6 Iron".to_string(),
            max_yard: "175".to_string()
        })
    );
}
